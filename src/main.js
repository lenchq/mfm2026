...
function home() {
  ...
  <section class="wrap section" id="directions"><div class="section-heading"><p class="eyebrow">01 / НАПРАВЛЕНИЯ</p><h2>НАПРАВЛЕНИЕ<br>ВЫБИРАЕШЬ <span>ТЫ</span></h2><p>Три разных пути.<br>Безграничные возможности.</p></div><div class="track-grid">${tracks.map((t, i) => `<a class="track-card" href="${trackUrl(t)}"><div class="track-image"><img loading="lazy" src="${import.meta.env.BASE_URL}${t.image}" alt="Направление: ${t.name}"><span class="number">0${i + 1}</span><span class="round-arrow ${t.color}">↗</span></div><div class="track-body"><h3>${t.name}</h3><p>${t.description}</p><span class="text-link">ПОДРОБНЕЕ →</span></div></a>`).join('')}</div></section>
